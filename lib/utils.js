 ...
 */
export function t(sentence, words = {}) {
  if (global.langCode !== 'en') {
    ...
    sentence = JSON.parse(language)[sentence] || sentence
  }

  if (words.length > 0) {
    sentence = words.reduce((accumulator, word) => accumulator.replace(/@@([a-zA-Z]+)@@/, word), sentence)
  }
 ...
 *  Request object.
 */
export function setGlobalLangCode(request) {
  global.langCode = getLangCodes().find(langCode => request.acceptsLanguages(langCode) === langCode)
}
